/**
 * DevExtreme (esm/__internal/scheduler/view_model/generate_view_model/generate_grid_view_model.js)
 * Version: 25.2.3
...
import {
    OptionManager
} from "./options/option_manager";
import {
    addCollector
} from "./steps/add_collector/add_collector";
import {
    addDirection
} from "./steps/add_direction";
import {
    addEmptiness
} from "./steps/add_emptiness";
import {
    addGeometry
} from "./steps/add_geometry/add_geometry";
import {
    addPosition
} from "./steps/add_position";
import {
    addSortedIndex
} from "./steps/add_sorted_index";
import {
    expandAllDayAllDayPanel,
    expandAllDayRegularPanel
} from "./steps/expand_all_day";
import {
    groupByGroupIndex
} from "./steps/group_by_group_index";
import {
    maybeSplit
} from "./steps/maybe_split";
import {
    snapToCells
} from "./steps/snap_to_cells";
import {
    sortByDuration,
    sortByGroupIndex,
    sortByStartDate
...
import {
    splitByParts
} from "./steps/split_by_parts/split_by_parts";
import {
    cropByVirtualScreen
} from "./steps/virtual_screen_crop";
import {
    filterByVirtualScreen
} from "./steps/virtual_screen_filter";
export const generateGridViewModel = (schedulerStore, items) => {
    ...
    const step2 = maybeSplit(items, hasAllDayPanel, ((entities, panelName) => {
        ...
        const positionInsideGroup = byGroup.map((group => {
            ...
            const innerStep0 = isMonthView || "allDayPanel" === panelName ? expandAllDayAllDayPanel(group, endDayHour, viewOffset) : expandAllDayRegularPanel(group);
            ...
            const innerStep3 = isMonthView || "allDayPanel" === panelName ? snapToCells(innerStep2, optionManager.getCells(panelName)) : innerStep2;
    ...
    const step4 = filterByVirtualScreen(step3, viewDataProvider, isVirtualScrolling);
