/**
 * DevExtreme (esm/__internal/viz/chart_components/multi_axes_synchronizer.js)
 * Version: 25.2.3
...
const linearConverter = br => ({
    transform: (v, b) => adjust(getLogExt(v, b, br.allowNegatives, br.linearThreshold)),
...
const logConverter = br => ({
    transform: (v, b) => adjust(raiseToExt(v, b, br.allowNegatives, br.linearThreshold)),
    getTicks(interval, tickValues, base) {
        const ticks = [];
        let tick;
...
const multiAxesSynchronizer = {
    synchronize(valueAxes) {
        each(getValueAxesPerPanes(valueAxes), ((_, axes) => {
            if (axes.length <= 1) {
                return
            }
