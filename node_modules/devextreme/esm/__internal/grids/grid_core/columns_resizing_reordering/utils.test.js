/**
 * DevExtreme (esm/__internal/grids/grid_core/columns_resizing_reordering/utils.test.js)
 * Version: 25.2.3
...
describe("getDraggingPanelBoundingRects", (() => {
    it("returns equal amount of elements in tuple", (() => {
        ...
        expect(null === result || void 0 === result ? void 0 : result.length).toBe(draggingPanels.length)
    }));
    it("returns the same dragging panel objects", (() => {
        const draggingPanels = [getDraggingPanelMock(), getDraggingPanelMock(), getDraggingPanelMock()];
        const result = getDraggingPanelBoundingRects(draggingPanels);
        expect(result).not.toBeNull();
        null === result || void 0 === result || result.forEach(((draggingPanelBoundingRect, index) => {
            expect(draggingPanelBoundingRect.draggingPanel).toBe(draggingPanels[index])
        }))
    }));
    it("returns filtered tuple without empty objects", (() => {
        const draggingPanels = [getDraggingPanelMock(), void 0, getDraggingPanelMock(), null];
        const result = getDraggingPanelBoundingRects(draggingPanels);
        expect(null === result || void 0 === result ? void 0 : result.length).toBe(draggingPanels.filter(Boolean).length)
    }));
    it("returns null if the result is empty array", (() => {
        const draggingPanels = [void 0, null];
        const result = getDraggingPanelBoundingRects(draggingPanels);
        expect(result).toBeNull()
    }));
    it("returns null if draggingPanels array is empty", (() => {
        const result = getDraggingPanelBoundingRects([]);
        expect(result).toBeNull()
    }));
    it("returns null if draggingPanels is null or undefined", (() => {
        expect(getDraggingPanelBoundingRects(void 0)).toBeNull();
