/**
 * DevExtreme (cjs/__internal/viz/funnel/label.js)
 * Version: 25.2.3
...
var _label = require("../../viz/series/points/label");
...
function getConnectorStrategy(options, inverted) {
    ...
    return {
        isLabelInside: () => !isOutsidePosition(options.position),
        getFigureCenter: getFigureCenter,
        prepareLabelPoints(bBox) {
        ...
        isHorizontal: () => true,
        findFigurePoint: figure => getFigureCenter(figure),
        adjustPoints: points => points.map(Math.round)
    }
}
...
const plugin = exports.plugin = {
    ...
    extenders: {
        ...
        _applySize() {
            ...
            const groupWidth = this._labels.map((label => {
                label.resetEllipsis();
        ...
        _change_TILING() {
            ...
            that._labels.forEach(((label, index) => {
                ...
                const coords = halfBorderWidth ? item.coords.map(((coord, index) => {
                    if (1 === index || 3 === index) {
    ...
    members: {
        ...
        _defaultLabelTextAlignment() {
            return this._getOption("rtlEnabled", true) ? "right" : "left"
        },
        _correctLabelWidth(label, item, options) {
            const isLeftPos = "left" === options.horizontalAlignment;
        ...
        _createLabels() {
            ...
            this._labels = that._items.map((item => {
