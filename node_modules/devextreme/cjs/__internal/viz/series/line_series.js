/**
 * DevExtreme (cjs/__internal/viz/series/line_series.js)
 * Version: 25.2.3
...
const lineMethods = {
    ...
    _areBordersVisible: () => false,
    _getDefaultSegment: segment => ({
        line: (0, _utils.map)(segment.line || [], (pt => pt.getDefaultCoords()))
    }),
    ...
    _applyStyle(style) {
        ...
        (0, _iterator.each)(this._graphics || [], ((_, graphic) => {
    ...
    _animate() {
        ...
        (0, _iterator.each)(that._graphics || [], ((i, elem) => {
            let complete;
            if (i === lastIndex) {
                complete = function() {
    ...
    _sortPoints: (points, rotated) => rotated ? points.sort(((p1, p2) => p2.y - p1.y)) : points.sort(((p1, p2) => p1.x - p2.x)),
    ...
    getSeriesPairCoord(coord, isArgument) {
        ...
        for (let i = 0; i < nearestPoints.length; i++) {
            ...
            if (1 === p.length) {
                ...
                ts.forEach((t => {
                    if (t >= 0 && t <= 1) {
                        const tmpCoord = (1 - t) ** 3 * p[0][oppositeCoordName] + 3 * (1 - t) ** 2 * t * p[1][bezierOppositeCoordName] + 3 * (1 - t) * t * t * p[2][bezierOppositeCoordName] + t * t * t * p[3][oppositeCoordName];
    ...
    getSeriesPairCoord(params, isArgument) {
        ...
        const isInsideInterval = (prevPoint, point, _ref) => {
            ...
            return (p1 = {
                x: x,
                y: y
            }, p2 = centerPoint, sqrt((p1.x - p2.x) ** 2 + (p1.y - p2.y) ** 2)) <= argAxis.getRadius() && min(prevPoint.x, point.x) <= x && max(prevPoint.x, point.x) >= x && min(prevPoint.y, point.y) <= y && max(prevPoint.y, point.y) >= y;
            var p1, p2
        };
        let coords;
