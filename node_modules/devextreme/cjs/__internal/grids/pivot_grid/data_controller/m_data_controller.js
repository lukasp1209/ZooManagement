...
class DataController {
    constructor(options) {
        this.createHeaderInfo = (headerItems, headerDescriptions, cellDescriptions, isHorizontal, options) => {
            const info = [];
            const depthSize = this._getHeaderItemsDepth(headerItems) || 1;
            const d = new _deferred.Deferred;
            this._getViewHeaderItems(headerItems, headerDescriptions, cellDescriptions, depthSize, options).done((viewHeaderItems => {
                this._fillHeaderInfo(info, viewHeaderItems, depthSize, isHorizontal, "tree" === options.layout);
                options.notifyProgress(1);
                d.resolve(info)
            }));
            return d
        };
        this._getHeaderItemsDepth = headerItems => {
            let depth = 0;
        ...
        this._createInfoItem = (headerItem, breadth, isHorizontal, isTree) => {
        ...
        this._addInfoItem = (info, options) => {
            const breadth = options.lastIndex - options.index || 1;
            const itemInfo = this._createInfoItem(options.headerItem, breadth, options.isHorizontal, options.isTree);
        ...
        this._isItemSorted = (items, sortBySummaryPath) => {
            let path;
            const item = items[0];
            ...
            if (stringValuesUsed && -1 !== sortBySummaryPath[0].indexOf("&[") && headerItem.key || !headerItem.key) {
                ...
                path = (0, _iterator.map)(items, (it => it.dataIndex >= 0 ? it.value : it.text)).reverse()
            }
            if ("GT" === item.type) {
                path = path.slice(1)
            }
        ...
        this._getViewHeaderItems = (headerItems, headerDescriptions, cellDescriptions, depthSize, options) => {
            ...
            const d = new _deferred.Deferred;
            (0, _deferred.when)(viewHeaderItems).done((items => {
                ...
                if (!hideTotals || "tree" === options.layout) {
                    this._addAdditionalTotalHeaderItems(viewItems, headerDescriptions, options.showTotalsPrior, "tree" === options.layout)
                }(0, _deferred.when)((0, _m_widget_utils.foreachTreeAsync)(viewItems, (items => {
                    const item = items[0];
        ...
        this._createHeaderItem = (childrenStack, depth, index) => {
        ...
        this._createViewHeaderItems = (headerItems, headerDescriptions) => {
        ...
        this._addMetricHeaderItems = (headerItems, cellDescriptions, options) => {
            (0, _m_widget_utils.foreachTree)(headerItems, (items => {
                const item = items[0];
                let i;
                if (!item.children || 0 === item.children.length) {
        ...
        this._addAdditionalTotalHeaderItems = (headerItems, headerDescriptions, showTotalsPrior, isTree) => {
        ...
        this._removeEmptyParent = (items, index) => {
        ...
        this._removeHiddenItems = headerItems => {
        ...
        this._fillHeaderInfo = (info, viewHeaderItems, depthSize, isHorizontal, isTree) => {
            let lastIndex = 0;
            let index;
            let depth;
    ...
    _update() {
        ...
        (0, _deferred.when)(this.createHeaderInfo(data.columns, columnFields, dataFieldsForColumns, true, columnOptions), this.createHeaderInfo(data.rows, rowFields, dataFieldsForRows, false, rowOptions)).always((() => {
            dataSource._changeLoadingCount(-1)
    ...
    getRowsInfo(getAllData) {
        ...
        if (scrollController && !getAllData && !isOnePredefinedRow) {
            ...
            this.foreachRowInfo(newRowsInfo, ((rowInfo, visibleIndex, rowIndex, columnIndex, realColumnIndex) => {
    ...
    getCellsInfo(getAllData) {
        ...
        return this.createCellsInfo(rowsInfo, columnsInfo, data, this._dataSource.getAreaFields("data"), this._options.dataFieldArea, texts.dataNotAvailable)
    }
    dispose() {
