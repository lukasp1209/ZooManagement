...
class XmlaStore {
        ...
        for (let i = expandIndex; i <= expandLevel; i += 1) {
            ...
            if (arg) {
                ...
                if (take) {
                    ...
                    arg = (0, _string.format)(MDX_ORDER_TEMPLATE, arg, sortBy, "desc" === field.sortOrder ? "DESC" : "ASC")
                }
    ...
    generateMdxCore(axisStrings, withArray, columns, rows, filters, slice, cubeName) {
        ...
        if (axisStrings.length) {
            ...
            mdxString = withString + (0, _string.format)(MDX_SELECT_TEMPLATE, this.generateFrom(this.generateAxisFieldsFilter(columns), this.generateAxisFieldsFilter(rows), this.generateAxisFieldsFilter(filters || []), cubeName), slice.length ? (0, _string.format)("WHERE ({0})", slice.join(",")) : "", select)
        }
        return mdxString
    }
