...
class Resizable extends _dom_component.default {
    ...
    _fitDeltaProportionally(delta) {
        ...
        if (!isFittedX()) {
            const x = this._fitIntoArea("x", (0, _math.fitIntoRange)(calculateWidth(), minWidth, maxWidth) - size.width);
            fittedDelta = {
                x: x,
        ...
        if (!isFittedY()) {
            const y = this._fitIntoArea("y", (0, _math.fitIntoRange)(calculateHeight(), minHeight, maxHeight) - size.height);
            fittedDelta = {
    ...
    _roundStrict(delta) {
        ...
        const separatorOffset = (stepValue, offsetValue) => (1 + .2 * sign(offsetValue)) % 1 * stepValue;
        const isSmallOffset = (offsetValue, stepValue) => Math.abs(offsetValue) < .2 * stepValue;
